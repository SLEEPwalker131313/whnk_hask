...
class ChargerAgent(OEFAgent):
    ...
    def prepare_contract(self, price):
        ...
        self._contract.action(self._api, 'addCharger', 2456766, [self._entity],
                              Address(self.chargers[0]), self.longitude*1000, self.latitude*1000, self.price, self.rate, self.max_count)
    ...
    def on_cfp(self, msg_id: int, dialogue_id: int, origin: str, target: int, query: CFP_TYPES):
        ...
        proposal = Description({"price_per_energy_percent": price, "digest": str(self._contract),
